...
import {Modal, Select, Table, Loader} from 'components'; // eslint-disable-line no-unused-vars
import {getTimes} from 'utils/dateTimePickerItems';

const colors = ['gray', 'blue', 'red', 'green', 'purple', 'orange', 'lime', 'cyan', 'violet', 'pink'];
const eventsColumns = [
  {
    key   : 'title',
    title : 'Event Name'
...
const AdminEvents = ({
  ...
  const {result: {data, loading, error}, fetchData} = useFetch('events'); // eslint-disable-line no-unused-vars
  ...
  return (
    <div>
      <section className="w-5/6 mx-auto flex items-center justify-between">
        ...
        <button
          className="px-5 py-2 bg-blue-500 rounded text-white font-medium text-md" onClick={() => {
      ...
      <Table
        ...
        data={sortBy(events, item => item.date)}
        onRowClick={item => {
              ...
              <div className="flex items-center justify-between mx-5">
                {
                  colors.map(color => (
                    <div
                      className={
                        classnames(`rounded cursor-pointer hover:opacity-70 transition bg-${color}-400 w-5 h-5`,
                          {'opacity-70 border border-2 border-gray-600' : selectedColor === color})}
                      key={color}
      ...
      <Modal
        ...
        modalContent={(
          <p> Are you sure you want to remove { selectedEvent?.title} ?</p>
        )}
